...
namespace ElementOutline
{
    ...
    internal class CmdUploadRooms : IExternalCommand
    {
        ...
        private static bool _debug_output = false;
        ...
        /// point are exported.
        /// </summary>
        private static bool _tessellate_curves = true;

        /// <summary>
        /// Never tessellate a curve
        /// shorter than this length.
        /// </summary>
        private const double _min_tessellation_curve_length_in_feet = 0.2;

        /// <summary>
        /// Conversion factor from foot to quarter inch.
        /// </summary>
        private const double _quarter_inch = 1.0 / (12 * 4);
        ...
        /// </summary>
        private static JtLoops GetRoomLoops(Room room)
        {
            SpatialElementBoundaryOptions opt = new SpatialElementBoundaryOptions();

            opt.SpatialElementBoundaryLocation = SpatialElementBoundaryLocation.Center; // loops closed
            ...
            foreach (IList<BoundarySegment> loop in loops)
            {
                ...
                foreach (BoundarySegment seg in loop)
                {
                    ...
                    Debug.Assert(null == q || q.IsAlmostEqualTo(p), "expected last endpoint to equal current start point");

                    q = seg.GetCurve().GetEndPoint(1);

                    if (_debug_output)
                    {
                        Debug.Print("{0} --> {1}", Util.PointString(p), Util.PointString(q));
                    }
                    if (null == p0)
                    {
                ...
                Debug.Assert(q.IsAlmostEqualTo(p0), "expected last endpoint to equal loop start point");
        ...
        private static List<Element> GetFurniture(Room room)
        {
            ...
            FilteredElementCollector familyInstances = new FilteredElementCollector(doc).WhereElementIsNotElementType().WhereElementIsViewIndependent()
        ...
        /// face in feet.
        /// </summary>
        internal static JtLoop GetLoop(Autodesk.Revit.Creation.Application creapp, Face face)
        {
            JtLoop loop = null;
            foreach (EdgeArray a in face.EdgeLoops)
            {
                ...
                foreach (Edge e in a)
                {
                    ...
                    // correctly oriented:
                    curve = e.AsCurveFollowingFace(face);
                    if (_debug_output)
                    {
                        p = curve.GetEndPoint(0);
                        q = curve.GetEndPoint(1);
                        Debug.Print("{0} --> {1} following face", Util.PointString(p), Util.PointString(q));
                    }
                    curves.Add(curve);
                ...
                //List<CurveLoop> loopsIfc  = new List<CurveLoop>( 1 );
                //loopsIfc.Add( loopIfc );
                //IList<CurveLoop> loopsIfcOut = ExporterIFCUtils
                //  .ValidateCurveLoops( loopsIfc, XYZ.BasisZ );
                // This is no longer needed if we use AsCurveFollowingFace instead of AsCurve:
                ...
                foreach (Curve curve in curves)
                {
                    ...
                    // and extend both lines.
                    p = curve.GetEndPoint(0);
                    Debug.Assert(null == q || q.IsAlmostEqualTo(p, 1e-04),
                      string.Format("expected last endpoint to equal current start point, not distance {0}",
                    ...
                    int n = -1;
                    if (_tessellate_curves && _min_tessellation_curve_length_in_feet < q.DistanceTo(p))
                    {
                ...
                Debug.Assert(q.IsAlmostEqualTo(p0, 1e-05),
                  string.Format("expected last endpoint to equal current start point, not distance {0}", p0.DistanceTo(q)));
            }
            return loop;
        }
        ...
        private static int AddLoops(Autodesk.Revit.Creation.Application creapp, JtLoops loops, GeometryObject obj, ref int nExtrusionAnalysisFailures)
        {
            int nAdded = 0;
        ...
        /// contour.
        /// </summary>
        private static JtLoops GetPlanViewBoundaryLoopsMultiple(Element e, ref int nFailures)
        {
        ...
        /// united together.
        /// </summary>
        internal static JtLoops GetPlanViewBoundaryLoopsGeo(Autodesk.Revit.Creation.Application creapp, GeometryElement geo, ref int nFailures)
        {
            Solid union = null;
        ...
        /// </summary>
        internal static JtLoops GetSolidPlanViewBoundaryLoops(Element e, bool transformInstanceCoordsToSymbolCoords, ref int nFailures)
        {
            Autodesk.Revit.Creation.Application creapp = e.Document.Application.Create;
            JtLoops loops = null;
            Options opt = new Options();
            ...
            if (null != geo)
            {
                ...
                if (e is FamilyInstance)
                {
                    if (transformInstanceCoordsToSymbolCoords)
                    {
                        ...
                        Transform t = Transform.CreateTranslation(-lp.Point);
                        Transform r = Transform.CreateRotationAtPoint(XYZ.BasisZ, -lp.Rotation, lp.Point);
                        geo = geo.GetTransformed(t * r);
                    }
                    else
                    {
                        Debug.Assert(1 == geo.Count<GeometryObject>(), "expected as single geometry instance");
                        Debug.Assert(geo.First<GeometryObject>() is GeometryInstance, "expected as single geometry instance");
                        geo = (geo.First<GeometryObject>() as GeometryInstance).GetInstanceGeometry();
                    }
                }
                loops = GetPlanViewBoundaryLoopsGeo(creapp, geo, ref nFailures);
            }
            if (null == loops || 0 == loops.Count)
            {
                Debug.Print("Unable to determine geometry for " + Util.ElementDescription(e) + "; using bounding box instead.");

                BoundingBoxXYZ bb;

                if (e is FamilyInstance)
                {
                    bb = (e as FamilyInstance).Symbol.get_BoundingBox(null);
                }
                else
                {
        ...
        internal static void ListLoops(Element e, JtLoops loops)
        {
            ...
            Debug.Print("{0} has {1}{2}", Util.ElementDescription(e), Util.PluralString(nLoops, "loop"), Util.DotOrColon(nLoops));
        ...
        /// </summary>
        public static void UploadRoom(IntPtr hwnd, Document doc, Room room)
        {
            BoundingBoxXYZ bb = room.get_BoundingBox(null);
            if (null == bb)
            {
                Util.ErrorMsg(string.Format("Skipping room {0} " + "because it has no bounding box.", Util.ElementDescription(room)));
                return;
            }
            ...
            Dictionary<string, JtLoop> furnitureLoops = new Dictionary<string, JtLoop>();
            // List of instances referring to symbols
            List<JtPlacement2dInt> furnitureInstances = new List<JtPlacement2dInt>(furniture.Count);
            int nFailures;
            foreach (FamilyInstance f in furniture)
            {
                ...
                {
                    nFailures = 0;
                    JtLoops loops = GetSolidPlanViewBoundaryLoops(f, true, ref nFailures);
                    if (0 < nFailures)
                    {
                        Debug.Print("{0}: {1}", Util.ElementDescription(f), Util.PluralString(nFailures, "extrusion analyser failure"));
                    }
                    ListLoops(f, loops);
                    if (0 < loops.Count)
                    {
                ...
                furnitureInstances.Add(new JtPlacement2dInt(f));
            }
            IWin32Window revit_window = new JtWindowHandle(hwnd);
            string caption = doc.Title + " : " + doc.GetElement(room.LevelId).Name + " : " + room.Name;
            Bitmap bmp = GeoSnoop.DisplayRoom(roomLoops, furnitureLoops, furnitureInstances);
            GeoSnoop.DisplayImageInForm(revit_window, caption, false, bmp);

            //DbUpload.DbUploadRoom( room, furniture, roomLoops, furnitureLoops );
        }
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            if (null == doc)
            {
                Util.ErrorMsg("Please run this command in a valid" + " Revit project document.");
            ...
            if (0 < ids.Count)
            {
                foreach (ElementId id in ids)
                {
                    ...
                    {
                        Util.ErrorMsg("Please pre-select only room" + " elements before running this command.");
                        return Result.Failed;
                    }
                }
            }
            ...
            if (null == ids || 0 == ids.Count)
            {
                ...
                try
                {
                    refs = sel.PickObjects(ObjectType.Element, new RoomSelectionFilter(), "Please select rooms.");
                }
            ...
            foreach (ElementId id in ids)
            {
                UploadRoom(hwnd, doc, doc.GetElement(id) as Room);
            }
